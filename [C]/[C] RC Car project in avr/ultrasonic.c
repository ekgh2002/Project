...
ISR(INT4_vect)
{
	...
	{
		ultrasonic_left_distance = (1000000.0 * TCNT3 * 1024 / F_CPU) / 58;	 //  1cm : 58us소요			 
	}
}
...
ISR(INT5_vect)
{
	...
	{
		ultrasonic_center_distance = (1000000.0 * TCNT3 * 1024 / F_CPU) / 58;	 //  1cm : 58us소요
	}
}
...
ISR(INT6_vect)
{
	...
	{
		ultrasonic_right_distance = (1000000.0 * TCNT3 * 1024 / F_CPU) / 58;	 //  1cm : 58us소요
	}
}
...
void distance_check(void)  //  delay 빼고 할때 필요?
{
	ultrasonic_trigger();
...
void ultrasonic_trigger()
{
	...
	//  _delay_ms 를 timer INT로 처리한다. delay 가 있어서는 안된다.
	// 현재는 demo code 지만 timer 로 처리해야 한다.
	}
	u_timer = u_timer%3;
}
