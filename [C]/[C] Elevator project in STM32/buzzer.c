...
STM32에서 주파수를 만들 때 3개의 레지스터를 설정한다.
 PSC(Prescaler)분주비, ARR(Peroid)주기, CCRx(Duty) duty비 설정
 다음과 같이 적용 하면 된다.
...
void buzzer_main()
{
   int divide_freq = 1600000;
    ...
    for (int i=0; i < 4; i++)
    {
		...
		__HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_1, divide_freq / happy_birthday[i] / 2);  // Duty를 50%로 설정 한다.
		HAL_Delay(300);
...
//
//    /* 음악 끝나고 3초 후 시작 */
//    HAL_TIM_PWM_Stop(&htim3, TIM_CHANNEL_1) ;  //  pwm 제어 stop
//    HAL_Delay(3000);
//    HAL_TIM_PWM_Start(&htim3, TIM_CHANNEL_1) ;
//
